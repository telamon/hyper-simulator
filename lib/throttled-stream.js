...
class BufferedThrottleStream extends Duplex {
  ...
  __write (dir, data, cb) {
    ...
    console.log('ENQUEUE', dir ? 'TX' : 'RX', data)
    buffer.push(data)
    cb(null)
  }
  ...
  tick (iteration, budget) {
    ...
    if (this.finishedA && this.finishedB) return { rx, tx, end: true, noop: true }
      ...
      if (!rxDone) {
        if (this.bufferA.isEmpty()) {
          ...
          if (n > budget + this.carryRx) {
            ...
            this.push(data)
            console.log('RXDEQUE', 'this.push:', data.toString())
          }
        }
      }
      ...
      if (!txDone) {
        if (this.bufferB.isEmpty()) {
          ...
          if (n > budget + this.carryTx) {
            ...
            this.out.push(data)
            console.log('TXDEQUE', 'out.push:', data.toString())
          }
        }
      }
    }
