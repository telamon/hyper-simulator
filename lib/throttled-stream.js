...
class BufferedThrottleStream extends Duplex {
  ...
  constructor (src, dst) {
    ...
    this._waitReadCloseA = null
    this._waitReadCloseB = null
  }
  ...
  __write (dir, data, callback) {
    ...
    // Ensure that null is only pushed once
    if (data === null) {
      let p = dir ? this._waitReadCloseB : this._waitReadCloseA
      if (!p) {
        p = defer(done => buffer.push({ data: null, callback: done }))
        if (dir) this._waitReadCloseB = p
        else this._waitReadCloseA = p
      }
      p.then(callback)
    } else {
